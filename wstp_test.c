...
int wstpTest P(( int _tp1, int _tp2));
...
static const char* evalstrs[] = {
	"wstpTest::usage = \"wstpTest[x, y] -> return x + y\"",
	(const char*)0,
	(const char*)0
};
...
int WSInstall( WSLINK wslp)
{
	int _res;
	_res = WSConnect(wslp);
	if (_res) _res = _definepattern(wslp, (char *)"wstpTest[i_Integer, j_Integer]", (char *)"{ i, j }", 0);
